...
class Becky(commands.Cog):
    ...
    async def check_unban(self, ctx):
        """Unbans users if audit log shows a ban reason containing 'becky' or 'boop' (case-insensitive)."""
        guild = ctx.guild
        keywords = ['becky', 'boop']
        ...
        if unbanned:
            ...
            await ctx.send("🚫 No users found in full audit log with matching ban reason.")
